...
### 4.3.1 JPEG 2000 Lossy 增强 ⏳(部分已实现)
- [x] 质量参数/量化控制
  - [x] 质量级别 (1-100)
  - [x] 可配置分解层数（小图自动裁剪）
...
- [x] 多质量层支持 (NumLayers)
- [ ] 速率失真优化 / 精确码率控制
  - [ ] T1 记录每个 coding pass 的 ΔD/ΔR（R-D 统计）
  - [ ] T2 基于 slope 的 R-D 截断与分层码率分配（lambda 搜索/截断）
  - [ ] 分层目标码率分配策略（多层预算拆分）
  - [ ] 将 TargetRatio 等接口接入 R-D 分配，缺数据时回退启发式
  - [ ] 多层/目标比/自定义步长的专项渐进解码测试
