...
// Verify ROI shift direction differs between MaxShift (shift background) and General Scaling (shift ROI).
func TestROIShiftDirectionByStyle(t *testing.T) {
	rect := &ROIParams{X0: 0, Y0: 0, Width: 4, Height: 4, Shift: 2}

	// MaxShift: ROI blocks keep full quality (shift=0), background gets shift.
	...
	if shift := maxEnc.roiShiftForCodeBlock(roiBlock); shift != 0 {
		t.Fatalf("MaxShift ROI block should have shift=0, got %d", shift)
	}
	if shift := maxEnc.roiShiftForCodeBlock(bgBlock); shift != 2 {
		t.Fatalf("MaxShift background block should have shift=2, got %d", shift)
	}

	// General Scaling: ROI blocks get shift, background stays 0.
